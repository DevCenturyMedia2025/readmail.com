...
def test_clasificador_documental_no_confunde_frase_negativa_con_orden():
    file_obj = document_classifier.UnifiedFile(
        name="factura.pdf",
    ...
    assert document_classifier.classify_document_type(file_obj) != "orden_compra"
...
def test_factura_con_orden_negada_y_ok_pendiente_no_se_aprueba(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        reademail,
        "build_unified_files",
        lambda service, message_id, attachments: ([pdf, xml], [], []),
    )
